package fr.inria.cedar.quotientSummary.oldsummaries.weak;
...
public class SQLChecksForWeakSummarization {
	...
	public void showCommonSources() throws SQLException {
		try(Statement stmt = conn.createStatement()) {
			...
			try (ResultSet rs = stmt.executeQuery(findCommonSource)) {
				while (rs.next()) {
			...
			try(ResultSet rs = stmt.executeQuery(findCommonTarget)) {
				while (rs.next()) {
					System.out.println(rs.getString(1) + " - " + rs.getString(2)); 
				}
			}
			...
			try (ResultSet rs = stmt.executeQuery(findSourceTarget)) {
				while (rs.next()) {
					System.out.println(rs.getString(1) + " - " + rs.getString(2)); 
				}
			}
