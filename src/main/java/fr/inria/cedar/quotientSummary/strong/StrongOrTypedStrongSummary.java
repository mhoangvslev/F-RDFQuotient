...
// May 11, 19:16 om: we used to have global replacement of the nodes, wherever they appeared.
// Usually a node only appears once in untypedNodes.
// However, in intermediary states, there may temporarily be several appearances, on different (sc, tc) pairs.
// Thus, it appears we should only replace locally, for the exact clique pairs where we want to replace.
// The alternative, broad replacement below is obsolete and left for reference only.
//		HashMap<Long, Long> nodeReplacements = new HashMap<Long, Long>();
//		if (replaceForS) {
//			// we gather the 0, 1 or 2 node replacements to do in this map, to make sure we know them before we make them
//			// and also that each node will be replaced by only one other node
//			if (!newRepS.equals(repS)){
//				nodeReplacements.put(repS, newRepS);
//			}
//		}
//		if (replaceForO){
//			if (!newRepO.equals(repO)){
//				if (nodeReplacements.get(repO)!= null){
//					if (!nodeReplacements.get(repO).equals(newRepO)){ // we would need to replace repS==repO with two distinct nodes
//						throw new IllegalStateException("Could not determine whom to replace with");
//					}
//				}
//				nodeReplacements.put(repO, newRepO);
//			}
//		}
